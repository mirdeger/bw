...
class Request:
    ...
    def __repr__(self):
        # return "["+self.method+"] " + self.url + " (From: " + str(self.parent) + ")"
        if not self:
...
class Graph:
    def __init__(self):
        self.nodes = []  # TODO: Might want a hash set here
        self.edges = []
    ...
    def toMathematica(self):
        # res = "data = {"
        # cons = [ ('"' + str(n1) + '" -> "' + str(n2) + '"') for (n1,n2) in self.edges ]
        # res += ",".join(cons)
        # res += "};"
        # res += "Graph[data, VertexLabels -> All]"
...
class Form:
    ...
    class SelectElement:
        ...
        def __eq__(self, other):
            # return (self.itype == other.itype) and (self.name == other.name) and (self.selected == other.selected)
            return (self.itype == other.itype) and (self.name == other.name)

        def __hash__(self):
            # return hash(hash(self.itype) + hash(self.name) + hash(self.selected))
            return hash(hash(self.itype) + hash(self.name))
    ...
    # For entire Form
    def __repr__(self):
        # s  = "Form("+str(len(self.inputs))+", " + str(self.action) + ", " + str(self.method) + ")"+str(self.inputs)
...
class Crawler:
    ...
    #
    #                 if has_csrf(attack_entry['injected'][0]):
    #                     print("CSRF + XSS!")
    #
    ...
    def attack(self):
        ...
        for (vector_type, vector) in forms_to_attack:
            ...
            if vector_type == "form":
                print("Vector type = form")
                # form_xss = self.attack_form(driver, vector, attack_lookup_table)
        ...
        for (vector_type, vector) in gets_to_attack:
            ...
            if vector_type == "get":
                print("Vector type = get")
                # get_xss = self.attack_get(driver, vector)
                # successful_xss = successful_xss.union(get_xss)
            get_c += 1

        # Finally try a single 404 attack
        # not_found_xss = self.attack_404(driver, attack_lookup_table)
        # successful_xss = successful_xss.union(not_found_xss)
        ...
        # for xss in successful_xss:
        #     if str(xss) in self.attack_lookup_table:
        #         (edge, parameter, payload) = self.attack_lookup_table[str(xss)]
        #         print(edge, parameter, payload)
        #     else:
        #         logging.error( str(xss) +  "was NOT found in table. Script injection works, but not sure from which vector")
    ...
    def next_unvisited_edge(self, driver, graph):
        ...
        # Print all forms left to visit
        # for edge in [edge for edge in graph.edges if edge.value.method == "form" and edge.visited == False]:
        #     cform = edge.value.method_data
        #     print(cform)
        #     for inputs in cform.inputs:
        #         print("--", inputs)
        #     print()
        ...
        if not list_to_use and 'prev_edge' in graph.data:
            ...
            if prev_edge.value.method == "form":
                ...
                # print("DONES:")
                # done_forms = self.attacked_forms

                # for (form, hits) in done_forms.items():
                #     print(form, hits)
                #     print(form.__hash__())
                ...
                if not (prev_form in self.attacked_forms):
                    ...
                    # else:
                    #    logging.info("Can not attack prev form")
            ...
            # elif prev_edge.value.method == "event":
            #     print("Prev  was event")
            #     if self.events_in_row < self.max_events_in_row:
            #         list_to_use = [edge for edge in graph.edges if edge.n1 == prev_edge.n2 and edge.value.method == "event" and  edge.visited == False]

            #         print("Skip reload!")
            #         if list_to_use:
            #             self.events_in_row += 1
            #             edge = list_to_use[0]
            #             event = edge.value.method_data
            #             execute_event(driver, event)
            #             edge.visited = True
            #             return edge
            else:
    ...
    def load_page(self, driver, graph):
        ...
        # logging.info("Depth (full, dom): (%s, %s)" % ( str(depth(edge)), str(dom_depth(edge)) ) )
        # print("Depth (full, dom): (%s, %s)" % ( str(depth(edge)), str(dom_depth(edge)) ) )
    ...
    def rec_crawl(self):
        ...
        try:
            ...
            for t in todo:
                try:
                    if t['function_name']:  # WIVET 19 results in empty name, TODO find why
        ...
        wait_json = driver.execute_script("return JSON.stringify(need_to_wait)")
        wait = json.loads(wait_json)
        # print("Time to wait, based on timeouts, ", wait)
        if wait:
            time.sleep(1)
